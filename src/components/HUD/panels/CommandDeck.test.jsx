...
describe('CommandDeck', () => {
  ...
  test('renders loading state initially', () => {
    ...
    render(<CommandDeck />);
    expect(screen.getByText(/Loading Stardust Treasury.../i)).toBeInTheDocument();
  });
  ...
  test('renders panel title "Stardust Treasury"', () => {
    useUserProfile.mockReturnValue({ profile: { ...mockProfileBase, token_pool: 1000 }, loading: false, error: null });
    useUserIntentions.mockReturnValue({ intentions: [], loading: false, error: null });
    render(<CommandDeck />);
    expect(screen.getByRole('heading', { name: /Stardust Treasury/i })).toBeInTheDocument();
  });

  test('calculates available stardust correctly with no intentions', () => {
    useUserProfile.mockReturnValue({ profile: { ...mockProfileBase, token_pool: 5000 }, loading: false, error: null });
    useUserIntentions.mockReturnValue({ intentions: [], loading: false, error: null });
    render(<CommandDeck />);
    expect(screen.getByText('5,000 Stardust')).toBeInTheDocument();
    expect(consoleWarnMock).not.toHaveBeenCalled(); // No warnings if data is present
  });

  test('calculates available stardust with intentions having defined tokens', () => {
    useUserProfile.mockReturnValue({ profile: { ...mockProfileBase, token_pool: 5000 }, loading: false, error: null });
    useUserIntentions.mockReturnValue({
      intentions: [
        { id: 'p1', name: 'Intention 1', used_tokens: 100, reserved_tokens: 50 }, // 150
        { id: 'p2', name: 'Intention 2', used_tokens: 200, reserved_tokens: 100 }, // 300
    ...
    expect(screen.getByText('4,550 Stardust')).toBeInTheDocument();
  ...
  test('uses mocked token_pool if profile.token_pool is undefined and shows warning', () => {
    ...
    expect(screen.getByText('10,000 Stardust')).toBeInTheDocument();
  ...
  test('uses mocked intention tokens if intention tokens are undefined and shows warning', () => {
    useUserProfile.mockReturnValue({ profile: { ...mockProfileBase, token_pool: 5000 }, loading: false, error: null });
    useUserIntentions.mockReturnValue({
      intentions: [{ id: 'p1', name: 'Intention Alpha', used_tokens: undefined, reserved_tokens: undefined }],
      loading: false, 
      error: null 
    });
    render(<CommandDeck />);
    ...
    expect(consoleWarnMock).toHaveBeenCalledWith(expect.stringContaining("Using mocked used_tokens or reserved_tokens"));
    expect(screen.getByText("(Note: Display includes estimated values)")).toBeInTheDocument();
    // Check that a Stardust value is rendered
    expect(screen.getByText(/\d{1,3}(,\d{3})* Stardust/)).toBeInTheDocument();
  });
  ...
  test('does not render intention list', () => {
    useUserProfile.mockReturnValue({ profile: { ...mockProfileBase, token_pool: 1000 }, loading: false, error: null });
    useUserIntentions.mockReturnValue({ intentions: [], loading: false, error: null });
    render(<CommandDeck />);
    expect(screen.queryByRole('list')).not.toBeInTheDocument(); // No ul/li for intentions
  });
  ...
  test('panel minimization works correctly', () => {
    ...
    // Content is identified by the specific text "Available Stardust:"
    let contentText = screen.getByText(/Available Stardust:/i);
    ...
    expect(screen.queryByText(/Available Stardust:/i)).not.toBeVisible();
    
    expect(screen.getByRole('heading', { name: /Stardust Treasury/i })).toBeVisible();
    expect(minimizeButton).toHaveAttribute('aria-label', /expand/i);

    fireEvent.click(minimizeButton);
    expect(screen.getByText(/Available Stardust:/i)).toBeVisible();
