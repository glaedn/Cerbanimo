  ...
  useEffect(() => {
    const fetchProjectsAndTasks = async () => {
      ...
      try {
        ...
        const projectResponse = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/projects/userprojects?userId=${userId}`, {
        ...
        const projectsWithTaskData = await Promise.all(
          fetchedProjects.map(async (proj) => {
            try {
              const tasksResponse = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/tasks/p/${proj.id}`, {
                headers: { Authorization: `Bearer ${token}` },
              });
