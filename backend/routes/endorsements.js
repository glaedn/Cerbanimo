...
export async function canEndorse(user_id, story_node_id) {
  ...
  if (recent.rowCount > 0) {
    ...
    if (minutesSince < ENDORSEMENT_COOLDOWN_MINUTES) {
      return {
        allowed: false,
        reason: `You must wait ${ENDORSEMENT_COOLDOWN_MINUTES - minutesSince} more minutes before endorsing again.`,
      };
    }
  }
...
router.post('/', async (req, res) => {
  const { story_node_id, emoji, badge, comment, multiplier = 1.0 } = req.body;
  ...
  try {
    ...
    const { rowCount } = await pool.query(
      'SELECT 1 FROM endorsements WHERE user_id = $1 AND story_node_id = $2',
      [user_id, story_node_id]
    );

    if (rowCount > 0) {
      return res.status(403).json({ error: 'Already endorsed this story node' });
    }
    ...
    await pool.query(
      `INSERT INTO endorsements (user_id, story_node_id, emoji, badge, comment, multiplier)
       VALUES ($1, $2, $3, $4, $5, $6)`,
