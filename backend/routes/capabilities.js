...
router.get('/', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ message: "No capabilities found for this category" });
    }
...
//Fetch the whole Capabilities table.
router.get('/all', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ message: "No capabilities found" });
    }
