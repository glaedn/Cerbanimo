...
class GuildHealthService {
  async calculateGuildMetrics(guildId) {
    ...
    const statsQuery = `
      SELECT
        (SELECT count(*) FROM tasks WHERE skill_id = $1 AND status LIKE '%unassigned') as open_tasks,
        (SELECT count(*) FROM tasks WHERE status LIKE '%unassigned') as total_open_tasks,
        (SELECT count(*) FROM tasks WHERE skill_id = $1 AND status = 'completed') as completed_tasks,
        (SELECT count(*) FROM tasks WHERE skill_id = $1 AND status NOT LIKE '%unassigned') as submitted_tasks,
        ...
        (SELECT count(*) FROM verification_events ve JOIN tasks t ON ve.task_id = t.id WHERE t.skill_id = $1 AND ve.result = 'verified') as verified_count,
    ...
    const statsRes = await pool.query(statsQuery, [skillId]);
