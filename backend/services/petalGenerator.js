...
const text = completion.choices[0].message.content;
console.log('LLM response (generateSubpetals):', text);

try {
  const parsedData = parseLLMJsonResponse(text);
  if (!parsedData.projects || !parsedData.petals || !Array.isArray(parsedData.projects) || !Array.isArray(parsedData.petals)) {
    throw new Error('LLM response is not in the expected format.');
  }
  return parsedData;
} catch (err) {
  console.error('Failed to parse LLM response for autoGeneratePetals:', text);
  throw new Error('Failed to parse subpetals from LLM output for autoGeneratePetals');
}
};