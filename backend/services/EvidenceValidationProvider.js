  ...
  async semanticReview({ requirement, items, bundle }) {
    ...
    if (requested === 'deterministic') {
      ...
      if (scenario === 'needs_more_evidence') {
        return { status: 'needs_more_evidence', provider: 'deterministic', message: 'Deterministic scenario requested more evidence.' };
      }
      if (scenario === 'manual_review') {
        return { status: 'manual_review_required', provider: 'deterministic', message: 'Deterministic scenario requested manual review.' };
      }
      if (scenario === 'malformed_output') {
        return { status: 'manual_review_required', provider: 'deterministic', message: 'Provider output could not be parsed safely.' };
      }
      return {
        status: 'passed',
